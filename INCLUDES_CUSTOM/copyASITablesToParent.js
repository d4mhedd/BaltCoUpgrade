...
function copyASITablesToParent(pFromCapID, pToCapId) {
	try{
			//
			...
			while (tai.hasNext()) {
				...
				if (numrows > 0) {
					if (tn.indexOf("MEETINGINFORMATION") && tn.indexOf("APFOSCHOOLINFORMATION")) {
		...
		logDebug("A JavaScript Error occured in custom function copyASITablesToParent: " + err.message + " In Line " + err.lineNumber);
	}
}
