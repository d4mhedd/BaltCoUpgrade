/*------------------------------------------------------------------------------------------------------/
| Program: RentalExemptionRenewal 1.0.js  Trigger: Batch    Client : Baltimore County, MD
| 
| Version 1.0 
|
...
//aa.env.setValue("BatchJobName", "Rental Registration Renewal");
//aa.env.setValue("emailAddress", "accelainfo@baltimorecountymd.gov")
	
*/

aa.env.setValue("emailAddress","accelainfo@baltimorecountymd.gov");  //FROM Email ID
...
//var autoInvoiceFees = "Y";    // whether or not to invoice the fees added  (GG 7/28/2013 - comment out)
var asiFieldsUsed = true; // whether ASI or TSI fields are referenced by script. If not, set to false to save time
/*------------------------------------------------------------------------------------------------------/
...
var appTypeType = "Rental Housing";			//   "Animal"  app type to process.  GG 7/28/2013 - changed to Rental Housing)
var appSubtype = "Exemption";			//   app subtype to process, *  (GG 7/29/2013 - added Registration)
var appStatus = "Issued";                             
var appCategory = "*";			//   app category to process, e.g. *  (GG 7/10/2013 - added NA)

var licenseStatus = "Active";	// "Active".  Statuses to process,comma separated (GG 7/10/2013 - changed to Active from Pending1) 

var lookAheadDays = "90";   //NOT a Param		// Number of days from today {90} (GG 7/10/2013 - added 90 days) 
var daySpan = "32";		  //NOT a Param	// Days to search {7} (GG 7/10/2013 - added 7 days) 

var newRenewalStatus =  "About To Expire";//  "About To Expire".  Change renewal status to this {GG 7/29/2013 changed from "About To Expire" to Renewal)
//var newAppStatus = "Pending";	// "Renewal".  Change App Status to this Pending (gg 7/17/2013 This will change the Application once in application acceptance status)
var editWorkflowTask = "License Status";// "License Issuance"  Open this task, set it to License Status (GG 7/29/2013)
var editWorkflowStatus = "Issued"; //   "Renewal"
...
var sysDate = aa.date.getCurrentDate();
//var someDate = new Date(); et change 6/22/2016  to not be used. to and from date use sysdate


//someDate.setDate(someDate.getDate() + lookAheadDays);  //setting day 90 days ahead (GG 8/5/2013)

var fromDate = sysDate;		// et use sysdate  start and end dates for batch query
var toDate = sysDate;
//var fromDate = someDate;		//  et changed 6/22/2016 to not used parameter somedate instead sysdate
//var toDate = someDate;			// et changed 6/22/2016 to not used parameter somedate instead sysdate


var emailAddress = getParam("emailAddress");	// email to send report
//var ccAddress = getParam("ccemailAddress");
/*----------------------------------------------------------------------------------------------------/
...
function expireLicenses(licStat)
{
	...
	logMessage("INFO", "Found " + myExp.length + " License with status " + licStat + "."); //gg 07132013 prints out number of license
	...
	for (zzz in myExp)  // for each b1expiration (effectively, each license app)
	{
		...
		var capId1 = capId.getID1();	 //DNU dont understand what's going on here 
		...
		//appType should be License/Rental Housing/Exemption/*
		...
		//This checks the App Status.  Verifying that the application is the status of AppStatus
		//App Status should be Issued
		var capStatus = cap.getCapStatus();
		if (appStatus != capStatus)
		{
			//logDebug("Ignored the License " + capIDshow + " because of Application Status.");
			ignoreCount2++;
			continue;
		...
		if (newRenewalStatus.length)
		{
			...
			expDateString = expDate.getMonth() + "/" + expDate.getDayOfMonth() + "/" + expDate.getYear(); 
	         ...
	         var emailSubj = "Baltimore County Rental Registration Exemption License About To Expire";
        var emailText = "";
        realemailAddress = "";
        ...
        emailText = "This is a reminder that your Rental Registration Exemption License will expire on " + expDateString + ". This is a system generated email. DO NOT REPLY.";
        emailText = emailText + "<br>" + "<br>";
                
        //ET turn off email 6/16
          //if (emailAddress.length) 
         {
            
           // aa.sendMail(emailAddress, realemailAddress, null, emailSubj, emailText);       
		...
		if (editWorkflowTask.length)
		{
			...
			if(appSubType=="Exemption")    //changed from pets GG 7/31/2013
			{ 
				wfCode = "LICRENT";  //changed from LICPET GG 7/31/2013
			}
			...
			if (editWorkflowStatus.length && wfCode != "") //then update statuses
			{
				...
				loopTask("License Status", "About To Expire", "Updated by Batch Job: "+batchJobName, "Updated by Batch Job: "+batchJobName, wfCode);
				//updateTask("Application Acceptance","Pending", "Updated by Batch Job: "+batchJobName, "Updated by Batch Job: "+batchJobName, capId);
	...
	logMessage("INFO","Processed " + capCount + " License of Status " + licStat + " out of " + licenseCount + " found.  # of License ignored due to Record Type: " + ignoreCount);
...
function taskEditStatus(wfstr,wfstat,wfcomment,wfnote,pFlow,pProcess) //Batch version of function
	...
	for (i in wfObjArray)
		...
		//**************************** FEES ELIMINATED ************************************************************
		//			USING DIFFERENT UPDATE TASK FUNCTION comment out 7/17/13 GG
		//*********************************************************************************************************
...
function loopTask(wfstr,wfstat,wfcomment,wfnote) // optional process name
	...
	for (i in wfObj)
		{
   		var fTask = wfObj[i];
		//logMessage("CHECKING " + wfstr.toUpperCase() + " " + fTask.getTaskDescription() + " " + processName + " " + fTask.getProcessCode() + " " +  useProcess);
			if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			{
